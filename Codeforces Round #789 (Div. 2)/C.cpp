...
void solve(){
    ...
    vi arr(n);
    fr(i,n)cin>>arr[i];
    vector<vector<int>> pehle(n+1,vector<int>(n,0));
    vector<vector<int>> baad(n+1,vector<int>(n,0));
    for(int i=n-1;i>=0;i--){
        for(int j=i-1;j>=0;j--){
            if(arr[j]<arr[i]){
                pehle[i][j]=pehle[i][j+1]+1;
            }else pehle[i][j]=pehle[i][j+1];
        }
    }
    for(int i=0;i<n;i++){
        for(int j=i+1;j<n;j++){
            if(arr[j]<arr[i]){
                baad[i][j]=baad[i][j-1]+1;
            }else baad[i][j]=baad[i][j-1];
        }
    }
    lli ans=0;
    for(int i=1;i<n;i++){
        for(int j=i+1;j<n-1;j++){
            ans+=(pehle[j][0]-pehle[j][i])*(baad[i][n-1]-baad[i][j]);
        }
    }
    cout<<ans<<endl;
}
