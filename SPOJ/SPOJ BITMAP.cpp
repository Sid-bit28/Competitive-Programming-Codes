...
lli n,m;
vector<string> s(mxN);
vector<vector<lli>> dis(mxN,vector<lli>(mxN,1e9));
int dx[]={1,0,-1,0};
int dy[]={0,1,0,-1};
void pre(){
...
bool check(int x,int y){
	if(x>=0&&x<n&&y>=0&&y<m&&s[x][y]=='0')return 1;
	return 0;
}
void solve(){
	cin>>n>>m;
	fr(i,n)cin>>s[i];
	dis= vector<vector<lli>> (n,vector<lli>(m,1e9));
	queue<pii> q;
	fr(i,n){
		fr(j,m){
			if(s[i][j]=='1'){
	...
	while(!q.empty()){
		...
		q.pop();
		fr(i,4){
			if(check(pos.F+dx[i],pos.S+dy[i])&&dis[pos.F+dx[i]][pos.S+dy[i]]>dis[pos.F][pos.S]+1){
				dis[pos.F+dx[i]][pos.S+dy[i]]=dis[pos.F][pos.S]+1;
