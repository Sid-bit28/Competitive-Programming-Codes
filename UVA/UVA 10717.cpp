	...
	while(1){
		...
		rep(i,0,n-1){
			rep(j,i+1,n-1){
				rep(k,j+1,n-1){
					rep(l,k+1,n-1){
						lli num1=(arr[i]*arr[j])/__gcd(arr[i],arr[j]);
						lli num2=(arr[k]*arr[l])/__gcd(arr[k],arr[l]);
						lli lcm=(num1*num2)/__gcd(num1,num2);
						rep(m,0,x-1){
							lli l2=length[m]/lcm*lcm;
							lli u2=(length[m]+lcm-1)/lcm*lcm;
							if(L[m]==-1||l2>L[m])L[m]=l2;
                            if(U[m]==-1||u2<U[m])U[m]=u2;
						}
					}
				}
			}
