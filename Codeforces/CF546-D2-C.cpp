...
void solve(){
	...
	deque<int> d1,d2;
	fr(i,k1){
		d1.push_back(player1[i]);
	}
	fr(i,k2){
		d2.push_back(player2[i]);
	}
	set<pair<vi,vi>> s;
	int cnt=0;
	while(!d1.empty()&&!d2.empty()){
		...
		cnt++;
		vi compare1,compare2;
		for(auto x:d1){
			compare1.push_back(x);
		}
		for(auto x:d2){
			compare2.push_back(x);
		}
		if(s.find({compare1,compare2})==s.end()){
			s.insert({compare1,compare2});
		}else {
			cout<<"-1"<<endl;
			return;
		}
